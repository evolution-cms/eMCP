...
return new class extends Migration {
    public function up(): void
    {
        ...
        $payload = [
            'name' => 'Dispatch eMCP tasks',
            'lang_key' => 'eMCP::global.permission_dispatch',
            'group_id' => $groupId,
            'disabled' => 0,
            'updated_at' => now(),
        ];

        $existing = DB::table('permissions')->where('key', 'emcp_dispatch')->first();
        if ($existing) {
            DB::table('permissions')->where('key', 'emcp_dispatch')->update($payload);
        } else {
            try {
        ...
        $exists = DB::table('role_permissions')
            ->where('role_id', 1)
            ->where('permission', 'emcp_dispatch')
            ->exists();
    ...
    public function down(): void
    {
        ...
        if (Schema::hasTable('permissions')) {
            DB::table('permissions')->where('key', 'emcp_dispatch')->delete();
        }
    }
